...
# Two-line prompt
PROMPT='%{$fg[cyan]%}%n %{$fg_bold[cyan]%}$ZSH_THEME_CLOUD_PREFIX  %{$fg_bold[green]%}%c %{$fg_bold[cyan]%}$(git_prompt_info)%{$reset_color%}
...
prompt_node_version_if_git() {
  if git rev-parse --is-inside-work-tree &>/dev/null; then
    if command -v node &>/dev/null; then
      echo "%{$fg[green]%}[%{$fg[cyan]%}node v$(node -v | sed "s/^v//")%{$fg[green]%}]%{$reset_color%}"
    fi
  fi
}
